...
def home():
    ...
    st.markdown("""<strong>Logistic Function:</strong> While linear regression models predict continuous values, a logistical regression model predicts probabilities. The outputs of the regression model are values between 0 and 1. This makes a logistical regression model perfect for binary classification tasks.""", unsafe_allow_html=True)
    st.markdown("""<strong>Probability to Classes:</strong> After we get this probability as an output we set a threshold (usually 0.5) to classify the probablility into two classes. In our instance that would mean that if the probablity that a transaction is fraudulent is greater than 50 percent, it would get flagged.""", unsafe_allow_html=True)
    st.markdown("""<strong>Model training:</strong> During the models training, it adjusts its parameters to best fit the data. This involves finding the best curve (example of curve below) that seperates the two classes by maximizing the likelihood of the observed data. This training takes place in a supervised environment.""", unsafe_allow_html=True)
...
def data_exploration():
    ...
    st.markdown("""
    The columns of our dataset consists of <span style="color: red;">28 anonymized Parameters</span>, 
...
def model_training():   
    ...
    the feature <span style="color: red;">Id</span> since it is <span style="color: red;">not important</span> 
    ...
    to the <span style="color: red;">same scale</span>.
    """, unsafe_allow_html=True)
    ...
    if st.button('Train Logistic Regression Model'):
        ...
        st.balloons()
        st.markdown("<h2 style='text-align: center;'>Model ran successfully!</h2>", unsafe_allow_html=True)
